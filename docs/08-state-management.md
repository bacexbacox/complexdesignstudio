...
- **Svelte 5 Global State**: Jangan gunakan fungsi `writable` (gaya Svelte 4) kecuali terpaksa. Untuk _state_ yang perlu dibagikan di berbagai tempat (misalnya data keranjang atau user auth), buat file `.svelte.ts` murni dan gunakan pola _Shared State_ atau _Context API_ Svelte (`setContext`, `getContext`) dengan objek reaktif `$state`.
...
- **Loading state lokal**: gunakan `$state(false)` untuk melacak loading di dalam suatu komponen, jangan gunakan _global store_ untuk hal kecil.
